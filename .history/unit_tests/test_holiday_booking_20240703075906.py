...
class TestHolidayBooking(unittest.TestCase):
    ...
    def test_invalid_request_id(self):
        response = self.post_request({"request_id": "", "params": {"name": "example", "country": "example", "age": "50"}})
        self.assertEqual(response.status_code, 422)
        data = response.json()
        self.assertIn('detail', data)

    def test_special_characters_in_name(self):
        response = self.post_request({"request_id": "test_id", "params": {"name": "!@#$%^&*", "country": "example", "age": "50"}})
        self.assertEqual(response.status_code, 200)
        data = response.json()
        self.assertTrue(data['complete'])
        self.assertIsNotNone(data['booking_id'])
    ...
    def test_large_age_value(self):
        response = self.post_request({"request_id": "test_id", "params": {"name": "example", "country": "example", "age": "999"}})
        self.assertEqual(response.status_code, 200)
        data = response.json()
        self.assertTrue(data['complete'])
        self.assertIsNotNone(data['booking_id'])
    ...
    def test_negative_age_value(self):
        response = self.post_request({"request_id": "test_id", "params": {"name": "example", "country": "example", "age": "-1"}})
        self.assertEqual(response.status_code, 400)
        data = response.json()
        self.assertIn('error', data)

    def test_long_name(self):
        response = self.post_request({"request_id": "test_id", "params": {"name": "a" * 256, "country": "example", "age": "50"}})
        self.assertEqual(response.status_code, 400)
        data = response.json()
        self.assertIn('error', data)

    def test_invalid_age(self):
        response = self.post_request({"request_id": "test_id", "params": {"name": "example", "country": "example", "age": "invalid"}})
        self.assertEqual(response.status_code, 400)
        data = response.json()
        self.assertIn('error', data)

    def test_empty_params(self):
        response = self.post_request({"request_id": "test_id", "params": {}})
        self.assertEqual(response.status_code, 200)
        data = response.json()
        self.assertTrue(data['complete'])
        self.assertIsNotNone(data['booking_id'])
