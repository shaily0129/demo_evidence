...
class TestHolidayBooking(unittest.TestCase):
    ...
    def test_invalid_age(self):
        response = self.post_request({"request_id": "string", "params": {"name": "marshall", "country": "UK", "age": "invalid"}})
    ...
    def test_invalid_request_id(self):
        response = self.post_request({"request_id": "", "params": {"name": "marshall", "country": "UK", "age": "78"}})
        self.assertEqual(response.status_code, 200)
        data = response.json()
        self.assertTrue(data['complete'])
        self.assertIn('error', data)
    ...
    def test_long_name(self):
        response = self.post_request({"request_id": "s1", "params": {"name": "a" * 256, "country": "UK", "age": "78"}})
        self.assertEqual(response.status_code, 200)
        data = response.json()
        self.assertTruedata['complete'])
        self.assertIn('error', data)
    ...
    def test_no_params(self):
        response = self.post_request({"request_id": "s1"})
        self.assertEqual(response.status_code, 200)
        data = response.json()
        self.assertFalse(data['complete'])
        self.assertIn('error', data)
